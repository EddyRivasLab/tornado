/* m3_train.c
 * Training program for milestone 3: 
 * Parameter estimation for a compiled generalized grammar,
 * from database of known structures.
 */
...
static char usage[] = "\
Usage: ./m2_train [-options] <gobfile> <infile> <outfile>\n\
 where <gobfile> is a compiled grammar file, <infile> is a\n\
 Stockholm format alignment file containing known individual\n\
 secondary structures, for training grammar on.\n\
...
{
  ESL_GETOPTS  *go;
  char         *gobfile;        /* grammar object (gob) file name */
  char         *seqfile;
  char         *outfile;
  ESL_ALPHABET *abc;
  ESL_SQFILE   *sqfp;
  ...
  gobfile = esl_opt_GetCmdlineArg(go, eslARG_STRING, NULL);
  ...
  G = read_compiled_grammar(gobfile);
  ...
  if (cfiles != NULL) 
    {
      Gct = read_compiled_grammar(gobfile);
  ...
  while ((status = esl_sq_Read(sqfp, sq)) == eslOK) 
      ...
      if (cfiles != NULL)
	{
      ...
      CYK_Destroy(cyk, G->M);
      fprintf(stderr, "about to free(sq->dsq)\n"); free(sq->dsq); sq->dsq = NULL;
      fprintf(stderr, "about to free(sq->ss)\n"); free(sq->ss);  sq->ss  = NULL; 
      esl_sq_Reuse(sq);
    }
  ...
  if (cfiles != NULL) 
    {
      Grammar_Destroy(Gct);
      fprintf(stderr, "about to free(csavefile)\n"); free(csavefile);
    }
